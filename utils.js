...
export async function fetchKalibiData() {
    const html = (await(await fetch("https://www.kalbi.pl/kalendarz-swiat-nietypowych")).text()); // html as text
    ...
    doc.querySelectorAll(".descritoptions-of-holiday").forEach(function (e) {
        ...
        e.querySelectorAll(".description-of-holiday").forEach(function (el) {
            ...
            desc.description = el.querySelector("p")?.textContent?.trim().replace(" ... »", "...");
...
export async function checkCredentials(req, client) {
    ...
    return !!res.rows.find(e => req.body.username == e.username && hashedPassword == e.password);
}
